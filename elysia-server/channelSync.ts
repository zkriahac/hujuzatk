    ...
    try {
      ...
      if (target) {
        ...
        try {
          ...
          // The (tenantId, externalId) unique key is tenant-wide, but the candidate
          // index above is scoped to this integration's (room, channel). So a UID that
          // already exists for the tenant OUTSIDE that scope — integration remapped to
          // a new room, two integrations sharing one feed, or the booking moved rooms —
          // misses the lookup and lands here. The channel is the source of truth for
          // placement, so re-link the existing row to THIS integration instead of
          // failing the whole event. Host-typed name/notes/phone are still preserved.
          if (createErr?.code !== 'P2002') throw createErr;
          const existing = await prisma.booking.findUnique({
            where: { tenantId_externalId: { tenantId, externalId: event.uid } },
            select: { id: true, room: true, guestName: true, notes: true, guestPhone: true },
          });
          if (!existing) throw createErr;
          if (existing.room !== integration.roomId) {
            console.log(`[${integration.channelName}] Re-linking ${event.uid} from room ${existing.room} → ${integration.roomId}`);
          }
          await prisma.booking.update({
            ...
            data: {
              ...
              externalChannel: integration.channelName,
              source: integration.channelName,
              guestName: looksAutoGenerated(existing.guestName) ? guestName : existing.guestName,
              guestPhone: looksAutoPhone(existing.guestPhone) ? (phoneStub || '') : existing.guestPhone,
              notes: looksAutoNotes(existing.notes) ? null : existing.notes,
              checkIn: event.start,
              checkOut: event.end,
              nights,
              status,
